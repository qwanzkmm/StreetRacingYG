...
public class AstronautThirdPersonCamera : NetworkBehaviour
{
    ...
    private bool isLocalPlayerCamera = false;

    private void Start()
    {
        if (hasAuthority)
        {
            isLocalPlayerCamera = true;
            camTransform.gameObject.SetActive(true); // Включаем камеру только для локального игрока
        }
        else
        {
            camTransform.gameObject.SetActive(false); // Отключаем камеру для удаленных игроков
        }
    }
